...
data_check=`hdfs dfs -du -s /user/hive/warehouse/${DS}{input%.*}.db/${DS}{input#$asterisk.}/ds=${DS}yesterday | awk -F' ' '{print ${DS}1}'`
if [[ ${DS}data_check -gt 0 ]]; then
    echo "In the script "${DS}0",Suceed in checking data of input:${DS}input on ${DS}{yesterday}."
else
    echo "In the script "${DS}0",Failed to obtain data of input:${DS}input on ${DS}{yesterday}!!!"
    exit -1
fi
...
set mapred.job.name=[HQL]${DS}{output};
...
data_check=`hdfs dfs -du -s /user/hive/warehouse/${DS}{output%.*}.db/${DS}{output#$asterisk.}/ds=${DS}yesterday | awk -F' ' '{print ${DS}1}'`
if [[ ${DS}data_check -gt 0 ]]; then
    echo "In the script "${DS}0",Suceed in producing data of output:${DS}output on ${DS}{yesterday}."
else
